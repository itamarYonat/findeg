...
window.FINDEG_PARSER = (function () {
  ...
  // opts (אופציונלי): { detectTrack(headerText) -> trackKey|null } - זיהוי
  // מסלול ספציפי-לפקולטה, ראו הערה על result.track למטה.
  function parseTranscript(lines, opts) {
    const result = {
    ...
    // זיהוי מסלול מכותרת התעודה - ספציפי לכל פקולטה, ולכן לא מקודד כאן יותר
    // (הפרסר משותף לכל הפקולטות, ראו faculties/<id>/detect.js). opts.detectTrack
    // (אופציונלי - window.FINDEG_FACULTY_DETECT.detectTrack, מוזרק מ-js/app.js)
    // מקבל את טקסט הכותרת ומחזיר מפתח-מסלול או null/undefined; בלי opts (או
    // בלי match) result.track נשאר null - המשתמש/ת בוחר/ת ידנית, בדיוק כמו היום.
    result.track = (opts && opts.detectTrack && opts.detectTrack(head)) || null;
  ...
  async function parsePdf(arrayBuffer, opts) {
    const lines = await extractLines(arrayBuffer);
    return parseTranscript(lines, opts);
  }

  // hasHeb מיוצא כדי ש-faculties/<id>/detect.js (זיהוי מסלול ספציפי-לפקולטה,
  // מוזרק כ-opts.detectTrack למעלה) יוכל לבדוק כותרות עברית בדיוק כמו כאן
  // (עמיד לטקסט הפוך - ראו ההערה על hasHeb למעלה) בלי לשכפל את הלוגיקה.
  return { parsePdf, parseTranscript, extractLines, hasHeb };
})();
