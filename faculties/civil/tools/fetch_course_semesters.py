...
Usage:
    python faculties/civil/tools/fetch_course_semesters.py

Output: course_semesters.json (full detail incl. both signals + confirmed
flag, alongside this script), and ../semesters.js (compact per-course
{seasons, confirmed} for the frontend).
"""
import json
import re
import sys
...
def main():
    data_js_text = (Path(__file__).parent.parent / "data.js").read_text(encoding="utf-8")
    our_ids = sorted(set(re.findall(r'"(\d{8})":\s*"', data_js_text)))
    recent_years = get_recent_years(data_js_text)
    print(f"Tracking {len(our_ids)} course ids from data.js")
    ...
    js_path = Path(__file__).parent.parent / "semesters.js"
    js_path.write_text("\n".join(js_lines) + "\n", encoding="utf-8")
